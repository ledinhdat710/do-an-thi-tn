...
<div class="panel panel-default khungbang">
	<div class="panel-heading">
		<h6 class="panel-title"><i class="fas fa-chalkboard-teacher"></i><b class="tbl">NHẬP THÔNG TIN ĐỀ THI</b></h6>
	</div>
	@if(count($errors)>0)
	...
	<form action="../suadethi/{{$dethi->id_de}}" method="POST">
		...
		<table class="table">
			<tr class="tbl">
				...
				<td>
					<input type="text" class="from-control nhaploai" value="{{$dethi->tendethi}}" placeholder="  Nhập tên đề thi" name="tendethi" id="txtTen" />
			...
			<tr class="tbl">
				...
				<td>
					<select name="tenkythi" id="tenloaich" class="form-control nhaploai">
						...
						@foreach($kythi as $kt)
						<option @if($dethi->id_ky == $kt->id_ky)
							{{"selected"}}
							@endif
							value="{{$kt->id_ky}}">{{$kt->tenky}}
						</option>
						@endforeach
			...
			<tr class="tbl">
				...
				<td>
					<select name="namekhoi" id="tenloaich" class="form-control nhaploai">
						...
						@foreach($khoi as $tk)
						<option @if($dethi->id_khoi == $tk->id_khoi)
							{{"selected"}}
							@endif
							value="{{$tk->id_khoi}}">{{$tk->tenkhoi}}
						</option>
						@endforeach
			...
			<tr class="tbl">
				...
				<td>
					<select name="namemonthi" id="tenloaich" class="form-control nhaploai">
						...
						<option @if($dethi->id_mh == $mt->id_mh)
							{{"selected"}}
							@endif
							value="{{$mt->id_mh}}">{{$mt->tenmh}}
						</option>
						@endforeach
			...
			<tr class="tbl">
				...
				<td>
					<select name="namegv" id="tenloaich" class="from-control nhaploai">
						...
						<option @if($dethi->id_gv == $gv->id_gv)
							{{"selected"}}
							@endif
							value="{{$gv->id_gv}}">{{$gv->hoten}}
						</option>
						@endforeach

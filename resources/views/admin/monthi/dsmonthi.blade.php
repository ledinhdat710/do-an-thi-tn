...
<div class="design_cauhoi">
    ...
    <!-- <a href="importmonthi"> <button type="button" style="background: #213351" class="btn btn-primary">Import</button></a>
...
<div class="panel panel-default">
    <div class="panel-heading">
        <h6 class="panel-title"><i class="fas fa-chalkboard-teacher"></i>QUẢN LÝ MÔN THI</h6>
    </div>
    <div class="datatable">
        @if(session('thongbao'))
        ...
        <table class="table tbl">
            ...
            <tbody>
                <?php
                $stt = 1;
                foreach ($monthi as $mt) : $stt ?>
                    <tr>
                        <td>{{ $stt++ }}</td>
                        <td>{{$mt->tenmh}}</td>
                        <td><a href="../suamonthi/{{$mt->id_mt}}">
                                <button data-toggle="modal" class="btn btnsuach">Sửa</button>
                            </a>
                        </td>
                        <td><a onclick="return confirm('Bạn có chắc chắn muốn xóa không?')" href="xoamonthi/{{$mt->id_mt}}">
